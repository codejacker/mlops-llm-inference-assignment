...
def _parse_json_object(text: str) -> dict:
    ...
    The verify prompt asks for strict JSON, but models still wrap it in prose or
    ```json fences. We strip a fence if present, grab the first {...} span, and
    json.loads it. On any failure we return {} so the caller can fall back to a
    safe default (treated as "not ok" -> the loop revises rather than crashes).
    """
...
def verify_node(state: AgentState) -> dict:
    ...
    # Defensive parse: if the model returns junk we default to ok=False, which
    # routes into a revise attempt rather than silently accepting a bad answer.
...
def revise_node(state: AgentState) -> dict:
    ...
    # Bump iteration so route_after_verify can enforce MAX_ITERATIONS and the
    # loop is guaranteed to terminate. Append to history so the eval runner can
    # re-score the SQL produced at each iteration.
    return {
        "sql": sql,
        "iteration": state.iteration + 1,
...
def route_after_verify(state: AgentState) -> str:
    ...
    """
    if state.verify_ok:
        return "end"  # verifier is happy - serve this answer
    if state.iteration >= MAX_ITERATIONS:
        return "end"  # out of budget - stop even though verify said not ok
    return "revise"  # not ok and budget remains - try to fix it
